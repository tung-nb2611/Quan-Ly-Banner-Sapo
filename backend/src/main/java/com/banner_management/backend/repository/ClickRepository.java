...
public interface ClickRepository extends JpaRepository<ClickEntity, Integer> {
    ...
    // query tinh luong view theo tháng theo khu vuc by minh
    @Query(value = "select count(id) from clicks where year(clicks.time_click) = ?1 and month(clicks.time_click) = ?2 ", nativeQuery = true)
    Integer getSumClickByMonth(int year, int month);
...
//    Integer getSumViewBySectionIDForWeek(int month, int sectionID);
    ...
    List<ClickEntity> getAllClikByBannerByID(Integer bannerID);
