...
import com.banner_management.backend.service.NumberClickService;
...
public class HomeController {
    ...
    public List<ClickBannerDto> getAll(){
        ...
        for(int i = 0 ; i < bannerEntityList.size() ; i ++){
            // lay so luong view
            List<ViewsEntity> viewsEntityList = viewsService.getByBannerID(bannerEntityList.get(i).getId());
            if(viewsEntityList.size() > 0){
                for(int j = 0 ; j < viewsEntityList.size() ; j ++){
                    numberViews += viewsEntityList.get(j).getNumber();
                }
            }
            else {
                numberViews = 0;
